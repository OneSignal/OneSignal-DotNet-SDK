...
public class iOSInAppMessagesManager : Core.InAppMessages.IInAppMessagesManager
{
    ...
    private sealed class InternalInAppMessageClickListener
        : Com.OneSignal.iOS.OSInAppMessageClickListener
    {
        private iOSInAppMessagesManager _manager;

        public InternalInAppMessageClickListener(iOSInAppMessagesManager manager)
            : base()
        {
            _manager = manager;
        }

        public override void OnClickInAppMessage(
            Com.OneSignal.iOS.OSInAppMessageClickEvent clickEvent
        )
        {
    ...
    private sealed class InternalInAppMessageLifeCycleListener
        : Com.OneSignal.iOS.OSInAppMessageLifecycleListener
    {
        private iOSInAppMessagesManager _manager;

        public InternalInAppMessageLifeCycleListener(iOSInAppMessagesManager manager)
            : base()
        {
            _manager = manager;
        }

        public override void OnWillDisplayInAppMessage(
            Com.OneSignal.iOS.OSInAppMessageWillDisplayEvent willDisplayEvent
        )
        {
            _manager.WillDisplay?.Invoke(_manager, GetWillDisplayArgs(willDisplayEvent));
        }

        public override void OnDidDisplayInAppMessage(
            Com.OneSignal.iOS.OSInAppMessageDidDisplayEvent didDisplayEvent
        )
        {
            _manager.DidDisplay?.Invoke(_manager, GetDidDisplayArgs(didDisplayEvent));
        }

        public override void OnWillDismissInAppMessage(
            Com.OneSignal.iOS.OSInAppMessageWillDismissEvent willDismissEvent
        )
        {
            _manager.WillDismiss?.Invoke(_manager, GetWillDismissArgs(willDismissEvent));
        }

        public override void OnDidDismissInAppMessage(
            Com.OneSignal.iOS.OSInAppMessageDidDismissEvent didDismissEvent
        )
        {
            _manager.DidDismiss?.Invoke(_manager, GetDidDismissArgs(didDismissEvent));
        }

        private InAppMessageWillDisplayEventArgs GetWillDisplayArgs(
            Com.OneSignal.iOS.OSInAppMessageWillDisplayEvent willDisplayEvent
        )
        {
        ...
        private InAppMessageDidDisplayEventArgs GetDidDisplayArgs(
            Com.OneSignal.iOS.OSInAppMessageDidDisplayEvent didDisplayEvent
        )
        {
        ...
        private InAppMessageWillDismissEventArgs GetWillDismissArgs(
            Com.OneSignal.iOS.OSInAppMessageWillDismissEvent willDismissEvent
        )
        {
        ...
        private InAppMessageDidDismissEventArgs GetDidDismissArgs(
            Com.OneSignal.iOS.OSInAppMessageDidDismissEvent didDismissEvent
        )
        {
